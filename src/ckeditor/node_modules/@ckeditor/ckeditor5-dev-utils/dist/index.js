...
async function getDllPluginWebpackConfig(webpack, options) {
    ...
    const { name: packageName } = fs$1.readJsonSync(path.join(options.packagePath, 'package.json'));
    const langDirExists = fs$1.existsSync(path.join(options.packagePath, 'lang'));
    const indexJsExists = fs$1.existsSync(path.join(options.packagePath, 'src', 'index.js'));
    const webpackConfig = {
        ...
        plugins: [
            ...
            new webpack.DllReferencePlugin({
                manifest: fs$1.readJsonSync(options.manifestPath),
                scope: 'ckeditor5/src',
                name: 'CKEditor5.dll'
            })
        ],
 ...
 */
function getPackageJson(cwd = process$1.cwd(), { async = false } = {}) {
    const path = upath.join(cwd, 'package.json');
    if (async) {
        return fs$1.readJson(path);
    }
    return fs$1.readJsonSync(path);
}
